...
#rename f later
f <- function(nodes,node=TRUE){
  if(node) toupper(gsub("^ *(.*[^ ]) *$","\\1",gsub("\\.","",sapply(nodes,xmlValue)))) else toupper(gsub("^ *(.*[^ ]) *$","\\1",gsub("\\.","",nodes)))
}
...
name_vector_sb <- f(c(name_vector_sb1,name_vector_sb2),node=FALSE)
name_vector_sb <- gsub(",.+","",name_vector_sb) #get ride of ,chair and ,vice chair
name_vector_sb
```
